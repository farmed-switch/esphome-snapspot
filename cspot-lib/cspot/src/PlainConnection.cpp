...
void PlainConnection::connect(const std::string& apAddress) {
  ...
  if (getaddrinfo(hostname.c_str(), portStr.c_str(), &h, &airoot) != 0) {
    CSPOT_LOG(error, "getaddrinfo failed for %s:%s", hostname.c_str(), portStr.c_str());
  ...
  for (ai = airoot; ai; ai = ai->ai_next) {
    ...
    if (::connect(this->apSock, (struct sockaddr*)ai->ai_addr,
      ...
      int flag = 1;
      setsockopt(this->apSock, IPPROTO_TCP, TCP_NODELAY,
