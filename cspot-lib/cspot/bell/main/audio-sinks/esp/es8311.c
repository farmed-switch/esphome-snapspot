...
static int Es8311WriteReg(uint8_t regAdd, uint8_t data) {
  ...
  res |= i2c_master_write_byte(cmd, ES8311_ADDR, 1  );
  res |= i2c_master_write_byte(cmd, regAdd, 1  );
...
int Es8311ReadReg(uint8_t regAdd) {
  ...
  res |= i2c_master_start(cmd);
  res |= i2c_master_write_byte(cmd, ES8311_ADDR | 0x01, 1  );
  res |= i2c_master_read_byte(cmd, &data, 0x01  );
  res |= i2c_master_stop(cmd);
  res |= i2c_master_cmd_begin(0, cmd, 1000 / portTICK_PERIOD_MS);
  i2c_cmd_link_delete(cmd);
...
static int Es7243WriteReg(uint8_t regAdd, uint8_t data) {
  ...
  i2c_cmd_handle_t cmd = i2c_cmd_link_create();
  res |= i2c_master_start(cmd);
  res |= i2c_master_write_byte(cmd, ES7243_ADDR, 1  );
  res |= i2c_master_write_byte(cmd, regAdd, 1  );
  res |= i2c_master_write_byte(cmd, data, 1  );
  res |= i2c_master_stop(cmd);
  res |= i2c_master_cmd_begin(0, cmd, 1000 / portTICK_PERIOD_MS);
...
int Es7243ReadReg(uint8_t regAdd) {
  ...
  cmd = i2c_cmd_link_create();
  res |= i2c_master_start(cmd);
  res |= i2c_master_write_byte(cmd, ES7243_ADDR | 0x01, 1  );
  res |= i2c_master_read_byte(cmd, &data, 0x01  );
  res |= i2c_master_stop(cmd);
  res |= i2c_master_cmd_begin(0, cmd, 1000 / portTICK_PERIOD_MS);
  i2c_cmd_link_delete(cmd);
...
int Es8311SetMicGain(MicGain gain) {
  ...
  gain_n |= gain / 6;
  res = Es8311WriteReg(ES8311_ADC_REG16, gain_n);
  return res;
}
...
int Es8311SetAdcVolume(uint8_t adc_vol) {
  int res = 0;
  res = Es8311WriteReg(ES8311_ADC_REG17, adc_vol);
  return res;
}
