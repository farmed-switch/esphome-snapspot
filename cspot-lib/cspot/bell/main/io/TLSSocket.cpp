...
void bell::TLSSocket::open(const std::string& hostUrl, uint16_t port) {
  ...
  if ((ret = mbedtls_net_connect(&server_fd, hostUrl.c_str(),
    ...
    BELL_LOG(error, "http_tls", "mbedtls_net_connect to %s:%d failed: -0x%04x %s",
             hostUrl.c_str(), port, -ret, errbuf);
    throw std::runtime_error(std::string("TCP connect failed: ") + errbuf +
                             " (-0x" + std::to_string(-ret) + ")");
  }
  ...
  while ((ret = mbedtls_ssl_handshake(&ssl)) != 0) {
    if (ret != MBEDTLS_ERR_SSL_WANT_READ && ret != MBEDTLS_ERR_SSL_WANT_WRITE) {
      char errbuf[128];
      mbedtls_strerror(ret, errbuf, sizeof(errbuf));
      BELL_LOG(error, "http_tls", "mbedtls_ssl_handshake failed: -0x%04x %s", -ret, errbuf);
      throw std::runtime_error(std::string("TLS handshake failed: ") + errbuf +
                               " (-0x" + std::to_string(-ret) + ")");
    }
  }
}
