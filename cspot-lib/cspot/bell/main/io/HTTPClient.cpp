...
void HTTPClient::Response::readResponseHeaders() {
  ...
  while (1) {
    ...
    size_t newBytes = socketStream.gcount();

    if (newBytes == 0) {
      throw std::runtime_error("Connection closed by server during HTTP response");
    }

    prevbuflen = httpBufferAvailable;
    httpBufferAvailable += newBytes;
    ...
    if (pret != -2)
      throw std::runtime_error("HTTP parser unexpected return value: " + std::to_string(pret));
