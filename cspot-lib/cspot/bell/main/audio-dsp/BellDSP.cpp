...
size_t BellDSP::process(uint8_t* data, size_t bytes, int channels,
  ...
  for (size_t i = 0; i < length16; i++) {
    dataLeft[i] = (data16Bit[i * 2] / (float)MAX_INT16);
    dataRight[i] =
        (data16Bit[i * 2 + 1] / (float)MAX_INT16);
  }
  ...
  for (size_t i = 0; i < length16; i++) {
    ...
    if (streamInfo->numChannels == 1) {
      data16Bit[i] = dataLeft[i] * MAX_INT16;
    } else {
      data16Bit[i * 2] = dataLeft[i] * MAX_INT16;
      data16Bit[i * 2 + 1] = dataRight[i] * MAX_INT16;
    }
  }
