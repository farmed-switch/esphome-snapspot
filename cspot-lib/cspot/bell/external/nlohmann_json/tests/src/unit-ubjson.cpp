    ...
    SECTION("individual values")
    {
        ...
        SECTION("number")
        {
            SECTION("signed")
            {
                ...
                SECTION("-2147483648..-32769 (int32)")
                {
                    ...
                    numbers.push_back(-2147483647 - 1);
                    for (auto i : numbers)
                    {
                        CAPTURE(i)
        ...
        SECTION("array")
        {
            ...
            SECTION("array with uint16_t elements")
            {
                ...
                {
                    json j(257, nullptr);
                    std::vector<uint8_t> expected(j.size() + 2, 'Z');
                    expected[0] = '[';
                    expected[258] = ']';
                    const auto result = json::to_ubjson(j);
                    CHECK(result == expected);
                ...
                {
                    json j(257, nullptr);
                    std::vector<uint8_t> expected(j.size() + 5, 'Z');
                    expected[0] = '[';
                    expected[1] = '#';
                    expected[2] = 'I';
                    expected[3] = 0x01;
            ...
            SECTION("array with uint32_t elements")
            {
                ...
                {
                    json j(65793, nullptr);
                    std::vector<uint8_t> expected(j.size() + 7, 'Z');
                    expected[0] = '[';
                    expected[1] = '#';
                    expected[2] = 'l';
                    expected[3] = 0x00;
    ...
    SECTION("errors")
    {
        ...
        SECTION("excessive size")
        {
            SECTION("array")
            {
                ...
                json j;
                nlohmann::detail::json_sax_dom_callback_parser<json> scp(j, [](int  , json::parse_event_t  , const json&  ) noexcept
                {
                    return true;
                });
    ...
    SECTION("parsing values")
    {
        ...
        SECTION("array")
        {
            SECTION("optimized version (length only)")
            {
                ...
                CHECK(json::to_ubjson(json::from_ubjson(v_C), true) == v_S);
            }
            ...
            SECTION("optimized version (type and length)")
            {
                ...
                CHECK(json::to_ubjson(json::from_ubjson(v_C), true, true) == v_S);
            }
        }
    }
