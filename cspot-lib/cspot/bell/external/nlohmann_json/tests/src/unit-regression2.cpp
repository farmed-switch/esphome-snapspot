...
{
  public:
    explicit Foo(const FooAlloc&   = FooAlloc()) {}
...
{
    for_3171_base(const std::string&   = {}) {}
...
struct for_3171_derived : public for_3171_base
{
    ...
    explicit for_3171_derived(const std::string&  ) { }
};
...
struct for_3204_foo
{
    ...
    explicit for_3204_foo(std::string  ) {}
};
...
struct for_3204_bar
{
    ...
    explicit for_3204_bar(std::function<void(for_3204_foo)>  ) noexcept
        : constructed_from(constructed_from_foo) {}
    explicit for_3204_bar(std::function<void(json)>  ) noexcept
...
TEST_CASE("regression tests 2")
{
    SECTION("issue #1001 - Fix memory leak during parser callback")
    {
        ...
        const json::parser_callback_t cb = [&](int  , json::parse_event_t event, json & parsed) noexcept
        {
    ...
    SECTION("issue #1045 - Using STL algorithms with JSON containers with expected results?")
    {
        ...
        {
            using comper_pair = std::pair<std::string, decltype(e1.value())>;
            return comper_pair(e1.key(), e1.value()) < comper_pair(e2.key(), e2.value());
        });
    ...
    {
        for_3204_bar bar_from_foo([](for_3204_foo) noexcept {});
        for_3204_bar bar_from_json([](json) noexcept {});
