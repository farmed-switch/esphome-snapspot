    ...
    SECTION("individual values")
    {
        ...
        SECTION("array")
        {
            ...
            SECTION("array with uint16_t elements")
            {
                ...
                std::vector<uint8_t> expected(j.size() + 3, 0xf6);
                expected[0] = 0x99;
                expected[1] = 0x01;
                expected[2] = 0x01;
                const auto result = json::to_cbor(j);
            ...
            SECTION("array with uint32_t elements")
            {
                ...
                std::vector<uint8_t> expected(j.size() + 5, 0xf6);
                expected[0] = 0x9a;
                expected[1] = 0x00;
                expected[2] = 0x01;
                expected[3] = 0x01;
        ...
        SECTION("binary")
        {
            ...
            SECTION("SAX callback with binary")
            {
                ...
                auto callback = [&binary_seen](int  , json::parse_event_t  , json & parsed) noexcept
                {
                    if (parsed.is_binary())
                    {
                        binary_seen = true;
...
TEST_CASE("CBOR roundtrips" * doctest::skip())
{
    SECTION("input from flynn")
    {
        ...
        exclude_packed.insert(TEST_DATA_DIRECTORY "/json_testsuite/sample.json");
...
TEST_CASE("Tagged values")
{
    ...
    SECTION("0xD8 - 1 byte follows")
    {
        SECTION("success")
        {
            ...
            auto v_tagged = v;
            v_tagged.insert(v_tagged.begin(), 0x42);
            v_tagged.insert(v_tagged.begin(), 0xD8);
    ...
    SECTION("0xD9 - 2 byte follow")
    {
        SECTION("success")
        {
            ...
            auto v_tagged = v;
            v_tagged.insert(v_tagged.begin(), 0x42);
            v_tagged.insert(v_tagged.begin(), 0x23);
            v_tagged.insert(v_tagged.begin(), 0xD9);
    ...
    SECTION("0xDA - 4 bytes follow")
    {
        SECTION("success")
        {
            ...
            v_tagged.insert(v_tagged.begin(), 0x42);
            v_tagged.insert(v_tagged.begin(), 0x23);
            v_tagged.insert(v_tagged.begin(), 0x22);
            v_tagged.insert(v_tagged.begin(), 0x11);
            v_tagged.insert(v_tagged.begin(), 0xDA);
    ...
    SECTION("0xDB - 8 bytes follow")
    {
        SECTION("success")
        {
            ...
            v_tagged.insert(v_tagged.begin(), 0x22);
            v_tagged.insert(v_tagged.begin(), 0x11);
            v_tagged.insert(v_tagged.begin(), 0xDB);
