...
TEST_CASE("compliance tests from nativejson-benchmark")
{
    ...
    SECTION("doubles")
    {
        ...
        TEST_DOUBLE("[0.017976931348623157e+310]", 1.7976931348623157e+308);
    ...
    SECTION("strings")
    {
        ...
        TEST_STRING(R"(["\"\\/\b\f\n\r\t"])", "\"\\/\b\f\n\r\t");
        TEST_STRING(R"(["\u0024"])", "$");
        TEST_STRING(R"(["\u00A2"])", "\xC2\xA2");
        TEST_STRING(R"(["\u20AC"])", "\xE2\x82\xAC");
        TEST_STRING(R"(["\uD834\uDD1E"])", "\xF0\x9D\x84\x9E");
    }
    ...
    SECTION("test_parsing")
    {
        ...
        SECTION("i -> n")
        {
                ...
                )
            {
                ...
                std::ifstream f(filename);
                json _;
                CHECK_THROWS_AS(_ = json::parse(f), json::exception&);
                std::ifstream f2(filename);
                CHECK(!json::accept(f2));
            }
        }
