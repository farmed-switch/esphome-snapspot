...
int test_repacketizer_api(void)
{
   ...
   VG_UNDEF(packet,4);
   if(opus_repacketizer_cat(rp,packet,0)!=OPUS_INVALID_PACKET)test_failed();
   cfgs++;
   packet[0]=1;
   if(opus_repacketizer_cat(rp,packet,2)!=OPUS_INVALID_PACKET)test_failed();
   cfgs++;
   packet[0]=2;
   if(opus_repacketizer_cat(rp,packet,1)!=OPUS_INVALID_PACKET)test_failed();
   cfgs++;
   packet[0]=3;
   if(opus_repacketizer_cat(rp,packet,1)!=OPUS_INVALID_PACKET)test_failed();
   cfgs++;
   ...
   packet[0]=2;
   packet[1]=250;
   if(opus_repacketizer_cat(rp,packet,251)!=OPUS_INVALID_PACKET)test_failed();
   cfgs++;
   packet[0]=3;
   packet[1]=0;
   if(opus_repacketizer_cat(rp,packet,2)!=OPUS_INVALID_PACKET)test_failed();
   cfgs++;
   packet[1]=49;
   if(opus_repacketizer_cat(rp,packet,100)!=OPUS_INVALID_PACKET)test_failed();
   cfgs++;
   packet[0]=0;
   if(opus_repacketizer_cat(rp,packet,3)!=OPUS_OK)test_failed();
   cfgs++;
   packet[0]=1<<2;
   if(opus_repacketizer_cat(rp,packet,3)!=OPUS_INVALID_PACKET)test_failed();
   cfgs++;
         ...
         for(k=0;k<=(1275+75);k+=3)
         {
            ...
            for(cnt=0;cnt<maxp+2;cnt++)
            {
               ...
               if(rcnt>0)
               {
                  ...
                  if((rcnt*i)<2&&(po[0]&3)!=0)test_failed();
                  if((rcnt*i)==2&&(po[0]&3)!=1)test_failed();
                  if((rcnt*i)>2&&(((po[0]&3)!=3)||(po[1]!=rcnt*i)))test_failed();
                  cfgs++;
                  ...
                  cfgs++;
               } else if (ret!=OPUS_BAD_ARG)test_failed();
            }
            opus_repacketizer_init(rp);
         }
      }
