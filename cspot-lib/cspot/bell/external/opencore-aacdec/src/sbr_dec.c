             ...
             tDec_Int_File  *pVars)
{
            ...
            (hParametricStereoDec != NULL))
    {
        ...
        for (i = 0; i < sbrDec->bufWriteOffs; i++)
        {
            ...
            pv_memmove(hFrameData->codecQmfBufferReal[i],
                       hFrameData->codecQmfBufferReal[j],
        ...
        for (i = 0; i < 32; i++)
        {
            ...
            if (pVars->mc_info.bDownSampledSbr)
            {
                calc_sbr_synfilterbank(hParametricStereoDec->qmfBufferReal[i],
                                       hParametricStereoDec->qmfBufferImag[i],
                                       ftimeOutPtr + (i << 6),
